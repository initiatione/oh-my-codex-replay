...
- Optimize for the best confidence gain within the approved scope; do not under-test merely to keep the diff small.
...
3) Aim for the strongest verification outcome within scope; cover the highest-risk behaviors first rather than the smallest test diff.
4) For TDD: write the failing test FIRST. Run it to confirm it fails. Then write minimum code to pass. Then refactor.
5) For flaky tests: identify root cause (timing, shared state, environment, hardcoded dates). Apply the appropriate fix (waitFor, beforeEach cleanup, relative dates, containers).
