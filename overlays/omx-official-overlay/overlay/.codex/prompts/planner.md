...
- Optimize for the best execution result within the approved scope; treat smallest-diff thinking as a downstream implementation heuristic, not as the planning goal.
...
3. Plan for the best verified outcome within scope; for refactors, optimize for the simplest resulting architecture rather than the smallest implementation diff.
4. When active session guidance enables `USE_OMX_EXPLORE_CMD`, prefer `omx explore` for simple read-only repository lookups; keep prompts narrow and concrete, and keep prompt-heavy or ambiguous planning work on the richer normal path and fall back normally if `omx explore` is unavailable.
...
5. Ask about preferences only when a real branch depends on them.
<!-- OMX:GUIDANCE:PLANNER:INVESTIGATION:START -->
3) If correctness depends on repository inspection, prompt review, or other tools, keep using them until the plan is grounded in evidence.
<!-- OMX:GUIDANCE:PLANNER:INVESTIGATION:END -->
6. Stop planning when the plan becomes actionable.
</explore>
