...
<scope_guard>
- Optimize for the best result that fully satisfies the task within scope; use the smallest viable diff as a heuristic for controlling risk, not as the primary goal.
- Do not broaden scope unless correctness requires it.
- Avoid one-off abstractions unless clearly justified.
- Surface important assumptions and tradeoffs before coding instead of silently choosing one path when ambiguity matters.
- Prefer the simplest implementation that achieves the desired outcome well; do not add speculative flexibility.
- Touch only code that traces directly to the task; remove only the dead code your own changes create.
- For refactor / cleanup / deslop tasks, prefer the simplest resulting architecture over the smallest diff, allow bounded structural changes that stay inside the refactor scope, and introduce only the abstractions needed to complete the restructuring coherently.
...
4. Define the concrete verification target first; for bugs or behavior changes, prefer a reproducer/test before editing when practical. For refactors, prefer locking current behavior with tests or concrete before/after checks before structural edits when practical.
5. Implement the simplest change that delivers the best verified result within scope.
6. Verify with diagnostics, tests, and build/typecheck when applicable.
