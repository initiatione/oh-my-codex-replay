...
- Optimize for the best grounded diagnosis and remediation path within scope; keep minimal-fix thinking as a guard against symptom patching, not as a reason to ignore the stronger root-cause fix.
</ask_gate>
...
- Fix recommendation is the strongest scope-appropriate root-cause fix, delivered one change at a time when practical
